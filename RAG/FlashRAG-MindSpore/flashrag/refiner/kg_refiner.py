...
class KGTraceRefiner(BaseRefiner):
    def __init__(self, config, retriever=None, generator=None):
        ...
        if config['dataset_name'].lower() == 'hotpotqa':
            from flashrag.prompt.trace_examplars import TRIPLE_EXAMPLARS_HOTPOTQA, GENERATING_CHAIN_EXAMPLARS_HOTPOTQA, FINAL_CHAIN_EXAMPLARS_HOTPOTQA
            ...
            from flashrag.prompt.trace_examplars import TRIPLE_EXAMPLARS_MUSIQUE, GENERATING_CHAIN_EXAMPLARS_MUSIQUE, FINAL_CHAIN_EXAMPLARS_MUSIQUE
            ...
            from flashrag.prompt.trace_examplars import TRIPLE_EXAMPLARS_2WIKIMULTIHOPQA, GENERATING_CHAIN_EXAMPLARS_2WIKIMULTIHOPQA, FINAL_CHAIN_EXAMPLARS_2WIKIMULTIHOPQA
        ...
        self.triple_examplars_embeddings = ms.tensor(self.triple_examplars_embeddings)
        ...
        self.chain_examplars_embeddings = ms.tensor(self.chain_examplars_embeddings)
    ...
    def get_examplars_for_triple(self, doc_list):
        ...
        doc_embeddings = ms.tensor(doc_embeddings)

        similarities = ops.matmul(doc_embeddings, self.triple_examplars_embeddings.T)
        examplars_rank = ops.argsort(similarities, axis=1, descending=True)
    ...
    def get_examplars_for_reasoning_chain(self, all_query):
        ...
        query_embeddings = ms.tensor(query_embeddings)

        similarities = ops.matmul(query_embeddings, self.chain_examplars_embeddings.T)
        examplars_rank = ops.argsort(similarities, axis=1, descending=True)
    ...
    def extract_document_triples(self, queries, retrieval_results):
        ...
        if len(docs_content) > 0:
            ...
            system_prompt = "Given a title and a text, extract all the knowledge triples in the form of <title; relation; tail entity>, "\
    ...
    def build_prompt_for_reasoning_chain(
        ...
        base_instruction = "Select the next knowledge triple that extends an existing set of knowledge triples to form a coherent reasoning path capable of answering a specified question. " \
        ...
        user_prompt = "The {hop}-th triple in the reasoning path is selected as:\nexisting knowledge triples: {existing_triples}\n"\
                    "question: {question}\ncandidate knowledge triples:\n{candidate_triples}\nthe next possible triple is:"
    ...
    def get_answer_token_indices(self, tokenizer, num_choices, token_ids):
        ...
        if self.token_id_to_choice_map is None:
            ...
            for choice in choices:
                ...
                self.token_id_to_choice_map[tokenizer.encode(" {}".format(choice), add_special_tokens=False)[-1]] = choice
        
        # answer_token_indices = ops.zeros((token_ids.shape[0],), dtype=token_ids.type()).fill_(token_ids.shape[1]-1)
        answer_token_indices = ops.fill(type=token_ids.type(),shape=(token_ids.shape[0],),value=token_ids.shape[1]-1)
    ...
    def get_reasoning_chain(self, all_query, all_doc_triples, triple_to_doc_ids):
        ...
        for query, doc_triples, generating_chain_examplars, final_chain_examplars, triple_doc_id_list in tqdm(
            zip(all_query, all_doc_triples, all_generating_chain_examplars, all_final_chain_examplars, triple_to_doc_ids),
            total=len(all_query), 
            ...
            flatten_doc_ids = sum(
                [
                    [doc_id for _ in single_doc_triple] for doc_id, single_doc_triple in zip(
                        triple_doc_id_list, doc_triples)
                ], 
                []
            )
            num_total_triples = len(flatten_triples)
            triple_text = ["<{}; {}; {}>".format(triple_item["head"], triple_item["relation"], triple_item["tail"]) for triple_item in flatten_triples]
            triple_embeddings = ms.tensor(self.encoder.encode(triple_text, is_query=False))
            ...
            for j in range(self.max_chain_length):
                ...
                path_queries = [
                    "knowledge triples: {}\nquestion: {}".format(
                        " ".join([triple_text[idx] for idx in path]), query
                    )
                    for path in paths
                ]
                path_query_embeddings = ms.tensor(self.encoder.encode(path_queries, is_query=True))

                path_triples_similarities = ops.matmul(path_query_embeddings, triple_embeddings.T)
                candidate_triples_mask = ops.ones_like(path_triples_similarities)
                ...
                topk_most_relevant_triples_indices = ops.topk(path_triples_similarities, k=min(self.topk_triple_select, num_total_triples), dim=1)[1].tolist()

                # construct prompts for selecting triple in reasoning chain
                # for each path, create an input to LLM
                input_prompts = []
                exisiting_triples = [[triple_text[idx] for idx in path] for path in paths] # each item represents existing triples path
                candidate_triples = [
                ...
                generated_token_ids, generated_token_logits = generate_output['generated_token_ids'], generate_output['generated_token_logits']

                answer_token_indices = self.get_answer_token_indices(self.generator.tokenizer, self.num_choices, generated_token_ids)
                answer_token_logits = generated_token_logits.gather_elements(1, \
                    answer_token_indices[:, None, None].broadcast_to((-1, -1, generated_token_logits.shape[-1])))
                answer_token_logits = answer_token_logits.squeeze(1)

                choices_token_ids_list = list(self.token_id_to_choice_map.keys())
                choices_list = [self.token_id_to_choice_map[token_id] for token_id in choices_token_ids_list] # ['A','B','C',..], may be duplication
                answer_token_probs = ops.softmax(answer_token_logits[:, choices_token_ids_list], axis=1)

                new_paths, new_paths_scores, new_paths_finished = [], [], []
                topk_choices_probs, topk_choices_indices = ops.topk(answer_token_probs, k=self.num_beams, dim=1) # for each path, select choice token in topk probs
                for i in range(len(paths)):
                    if paths_finished[i]:
                        ...
                        continue 
                    if ops.all(ops.isnan(topk_choices_probs[i])):
                        print("No choice in generated results! generated text: {}".format(self.generator.tokenizer.decode(generated_token_ids[i])))
                        new_paths.append(paths[i])
                        new_paths_scores.append(paths_scores[i])
                        new_paths_finished.append(False)
                        continue 
                    for b in range(self.num_beams):
                        if ops.isnan(topk_choices_probs[i, b]) or topk_choices_probs[i, b].item() < self.min_triple_prob:
                            continue
    ...
    def batch_run(self, dataset):
        ...
        triple_to_doc_ids = [[doc_item['id'] for doc_item in query_retrieval_result] for query_retrieval_result in retrieval_results]
        ...
        if self.context_type == 'triples':
            ...
            for query_chain_result, query_retrieval_results in zip(
                reasoning_chain_result, retrieval_results):
