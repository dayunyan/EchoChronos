...
def cross_attention_forward(
    ...
    q = self.q(input).view(bsz, -1, n_heads, d_heads).swapaxes(1, 2)
    if past_key_value_state == None:
        k = self.k(kv).view(bsz, -1, n_heads, d_heads).swapaxes(1, 2)
        v = self.v(kv).view(bsz, -1, n_heads, d_heads).swapaxes(1, 2)
    else:
        k, v = past_key_value_state

    scores = ops.einsum("bnqd,bnkd->bnqk", q, k)
    ...
    attn = ops.softmax(scores.float(), axis=-1).type_as(scores)
