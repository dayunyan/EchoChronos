...
import numpy as np
from mindnlp.transformers import AutoTokenizer, PreTrainedTokenizer, PreTrainedTokenizerFast
    ...
    def run_item(self, item):
        ...
        while gen_length < self.max_generation_length and iter_round < self.max_iter_num:
            ...
            if not judge_result:
                ...
                import langid
                for i in range(len(retrieval_result[0])):
                    if langid.classify(retrieval_result[0][i]['contents'])[0] not in ('zh','en'):
                        retrieval_result[0][i]['contents'] = 'empty docs'
