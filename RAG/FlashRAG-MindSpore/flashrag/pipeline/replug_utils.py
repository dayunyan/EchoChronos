...
from mindnlp.transformers.generation.stopping_criteria import validate_stopping_criteria
from mindnlp.transformers.generation.utils import (
    SampleDecoderOnlyOutput,
    SampleEncoderDecoderOutput,
    SampleOutput,
)
import mindnlp.transformers as transformers_
...
class REPLUG_Generation(GenerationMixin):
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        eos_token_id_tensor = (
            ms.tensor(eos_token_id) if eos_token_id is not None else None
        )
        output_scores = (
        ...
        unfinished_sequences = ops.ones(input_ids.shape[0], dtype=ms.Int)
        ...
        while True:
            if synced_gpus:
                ...
                all_reduce_sum = ops.AllReduce(ops.ReduceOp.SUM)
                this_peer_finished_flag = all_reduce_sum(this_peer_finished_flag)
            ...
            next_tokens = ops.multinomial(probs,2,False).squeeze(-1)
...
class REPLUGLogitsProcessor(LogitsProcessor):
    ...
    Reference: fastRAG
    """
    def __init__(self, doc_scores: ms.Tensor):
        self.num_docs = doc_scores.shape[0]
        # normalize
        doc_scores /= doc_scores.sum()
        self.doc_scores = ops.unsqueeze(doc_scores, 1) # k*1
    ...
    def __call__(self, input_ids, scores):
        ...
        replug_scores = ops.tile(replug_scores, (self.num_docs, 1)) # k*vocab_size
...
def load_replug_model(name_or_path):
        ...
        def factory(name_or_path):
            ...
            try:
                pretrained_class_object = getattr(transformers_, loadedConfig.architectures[0])
