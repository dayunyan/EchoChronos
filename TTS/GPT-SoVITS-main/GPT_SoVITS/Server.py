...
# gpt_path = os.path.join(parent_dir, "GPT_weights_v2", "xxx-e15.ckpt")
# sovits_path = os.path.join(parent_dir, "SoVITS_weights_v2", "xxx_e12_s168.pth")

# gpt_path = os.path.join(parent_dir, "GPT_weights_v2", "zgl-e15.ckpt")
# sovits_path = os.path.join(parent_dir, "SoVITS_weights_v2", "zgl_e12_s144.pth")
...
def get_tts_wav(character, 
    ...
    if not ref_free:
        with torch.no_grad():
            ...
            print("Prompt Semantic Shape:", prompt_semantic.shape)
    ...
    for i_text,text in enumerate(texts):
        ...
        # print("norm_text2:", norm_text2)
        # print("BERT2 example values:", bert2[:5, :5])  # 打印前 5x5 的数值
...
def tts_api():
    ...
    Body 参数:
    - ref_wav: 参考音频 (file)
    - prompt_text: 参考文本 (string)
    - prompt_language: 参考文本语言 (string)
    - text: 目标文本 (string)
    - text_language: 目标文本语言 (string)
    - how_to_cut: 文本切分方式 (string, 默认值: "不切")
    - top_k: Top-K 参数 (int, 默认值: 20)
    - top_p: Top-P 参数 (float, 默认值: 0.6)
    - temperature: 温度参数 (float, 默认值: 0.6)
    - ref_free: 是否使用参考音频 (bool, 默认值: False)
    - speed: 语速 (float, 默认值: 1.0)
    ...
    try:
        ...
        with wave.open(wav_io, 'wb') as wav_file:
            ...
            wav_file.writeframes(audio_opt_temp.tobytes())  # 写入音频数据
...
# text = "我是猪八戒，师傅啊，什么时候才能回高老庄啊"  # 目标文本
